...
exports.criar = async (req, res, next) => {
  try {
    ...
    if (destaque) {
      await prisma.videoUniversidade.updateMany({ data: { destaque: false } });
    }
    const video = await prisma.videoUniversidade.create({
      data: {
        titulo: titulo.trim(),
...
exports.atualizar = async (req, res, next) => {
  try {
    ...
    if (destaque) {
      await prisma.videoUniversidade.updateMany({ where: { id: { not: id } }, data: { destaque: false } });
    }
    const video = await prisma.videoUniversidade.update({
      where: { id },
      data: {
